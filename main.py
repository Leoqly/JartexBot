...
def create_card(stats):
    try:
        ...
        # Overlay per i box scuri (Effetto Vetro)
        overlay = Image.new("RGBA", base.size, (0,0,0,0))
        d_ov = ImageDraw.Draw(overlay)
        # Box statistiche a sinistra
        ...
        # SKIN 3D (Corretta per apparire a destra)
        try:
            skin_url = f"https://visage.surgeplay.com/full/400/{stats['username']}"
            s_res = requests.get(skin_url, timeout=7)
            skin = Image.open(io.BytesIO(s_res.content)).convert("RGBA")
            # Incolliamo la skin nell'angolo in alto a destra
            base.paste(skin, (base.width - 320, 10), skin)
        except: pass
...
async def stats(ctx, user: str):
    ...
    if buf: await ctx.send(file=discord.File(buf, f"{user}_stats.png"))
